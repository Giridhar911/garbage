...
int main()
{
    ...
    printf("Enter source (A, B, C, D, or E): ");
    src = toupper(getchar()) - 'A';
    getchar();  // Consume newline character

    printf("Enter destination (A, B, C, D, or E): ");
    ...
    do
    {
        ...
        for (i = 0; i < NON; i++)
        {
            ...
            {
                if (nodes[workn].wt + table[workn][i] < nodes[i].wt || nodes[i].wt == -1)
                {
        ...
        for (i = 0; i < NON; i++)
        {
            if (nodes[i].state == TEMP && (min == -1 || nodes[i].wt < min))
            {
                min = nodes[i].wt;
                workn = i;
            }
